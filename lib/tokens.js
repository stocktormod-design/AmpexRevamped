...
const colors = {
  // Backgrounds — lyshetsstige med SYNLIGE trinn.
  // Modellen er tonet grunn + hvite kort som flyter, ikke hvit grunn + kremkort.
  // Før lå bakgrunn/kort/chip/hårlinje mellom L*89 og L*100 — fire strukturelle
  // nivåer presset inn i elleve lyshetspoeng, og da separerer ingenting. Det var
  // årsaken til at flatene «forsvant i hverandre», ikke at temaet var for lyst.
  canvas: '#EFEAE1', // sidegrunn — varm sand, L*~92. Kun skjermrot.
  bg: '#FFFFFF', // KORTFLATE (ikke sidegrunn) — hvite kort løftes av grunnen
  groupedBg: '#F2F2F7', // DØD — ingen skjermer bruker den lenger (kald grå kolliderer med varm palett). Beholdt til tailwind-konsumenter er sjekket.
  fill: '#E5DDCE', // icon chips, shortcut cards, input fills — dyp nok til å lese innfelt mot hvitt kort
  fillPressed: '#D8CEBB', // fill while pressed
  ...
  // Lines
  separator: '#DED6C7', // hairline between list rows
  border: '#CDC4B1',
  ...
  // ── VERKTØYFLATER (mørke) ────────────────────────────────────────────────
  //
  // Kremfargen over er PAPIR. Den ligger i samme familie som Kindle (#FBF0D9),
  // Apple Notes (#FFFBED), Goodreads (#F4F1EA) og Panera (#F4EFE1) — altså
  // lese- og notatapper. Det er grunnen til at skjermene leste som notatblokk:
  // paletten sier «dokument», og alle andre bruker den til nettopp det.
  //
  // Regelen som følger av domenet:
  //   MØRKT  = verktøyet   — ordre, lager, prosjekter, lister. Tett og teknisk.
  //   KREMET = papiret     — skjema, tilbud, arkiv. Ting som ER dokumenter.
  //
  // Dybde lages med VERDI, ikke med skygge: lysere flate = nærmere. Det er slik
  // mørke grensesnitt faktisk bygges, og det fjerner samtidig den diffuse
  // 0.08-skyggen som ligger på alt.
  toolBg: '#211C15', // sidegrunn
  toolRaised: 'rgba(255,255,255,0.055)', // panel som ligger på grunnen
  toolRaisedStrong: 'rgba(255,255,255,0.10)', // valgt/aktivt panel, input
  toolBorder: 'rgba(255,255,255,0.10)', // hårlinje — erstatter skygge
